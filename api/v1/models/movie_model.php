...
class movie_Model extends Model
{
    ...
    public function readone_old($where)
    {
        ...
        //$dir       = '/jpstream/datshin-backend-api/uploads/';
        $dir       = '/backend/uploads/';
        $path = $server . $dir;
    ...
    public function read($where = null, $current = null, $limit = null)
    {
        ...
        if ($limit !== null && $current !== null) {
            ...
            $query = "SELECT  m.id as movie_id FROM movies as m ORDER BY m.modified_datetime DESC LIMIT $firstlimit,$secondlimit ";
            $stmt = $this->db->prepare($query);
        } elseif ($limit !== null && $current == null) {

            $query = "SELECT   m.id as movie_id FROM movies as m ORDER BY m.modified_datetime  DESC LIMIT $limit";
            $stmt = $this->db->prepare($query);
        } elseif ($current !== null) {
            ...
            $query = "SELECT   m.id as movie_id FROM movies as m ORDER BY m.modified_datetime DESC LIMIT 20";
            $stmt = $this->db->prepare($query);
        }
